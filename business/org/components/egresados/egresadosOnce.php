<?php  
    ...
    //////////// SECCION DEL TITULO
    $res_sentecia = $mysqli1->query($sentencia . "140");
    ...
    while ($row_datos_seccion = $res_seccion_dos->fetch_assoc()) {
          ...
          $html .= '
               <main class="container egresados-section">
                    <div class="row">
                         <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                              <h1 class="egresados-title">'.$row_datos_seccion['titulo'].'<br><span class="egresados-subtitle">'.$row_datos_seccion['subTitulo'].'</span></h1>
                         </div>
                    </div>
               </main>
          ';
    ...
    ///////////////// SECCION EGRESADOS ONCE
    $res_sentecia = $mysqli1->query($sentencia . "141");
    while ($row_sentencia = $res_sentecia->fetch_assoc()) {
        $sql_seccion_dos = $row_sentencia['campos'] . $row_sentencia['tablas'] . $row_sentencia['condiciones'];
    }
    $res_seccion_dos = $mysqli1->query($sql_seccion_dos);
    ...
    // Obteniendo las imagenes necesarios.
     $res_sentencia = $mysqli1->query($sentencia."145");
          ...
          foreach ($imagenes as $imagen) {
               ...
               if ($totalCargado == count($imagenes) && $imagenActual - 4 != 0) {
                    ...
                    for ($i = 0; $i < $restantes; $i++) {
                         $html .= '<div class="col-lg-3 col-md-0 col-sm-0 col-0"></div>';
                    }
               }
